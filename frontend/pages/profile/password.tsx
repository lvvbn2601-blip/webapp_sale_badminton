...
export default function ProfilePasswordPage() {
  ...
  const [activeTab, setActiveTab] = useState<"profile" | "address" | "password" | "purchases" | "notifications" | "vouchers">("password");
            ...
            <div className="rounded-2xl bg-white shadow-sm ring-1 ring-black/5 overflow-hidden">
              ...
              {activeTab === "password" && (
                <div className="p-6 sm:p-8 space-y-8 animate-in fade-in duration-300">
                  <h3 className="flex items-center gap-2 font-heading text-xl font-semibold border-b border-black/5 pb-4">
                  ...
                  <div className="space-y-5 max-w-sm">
                    <div className="space-y-1.5">
                      ...
                      <input type="password" value={oldPassword} onChange={(e) => setOldPassword(e.target.value)} placeholder="Enter old password" className="w-full rounded-xl border border-black/10 bg-gray-50/50 px-4 py-3 text-sm focus:border-primary focus:bg-white focus:outline-none focus:ring-4 focus:ring-primary/10" />
                    </div>
                    <div className="h-px bg-black/5 w-full my-2"></div>
                    <div className="space-y-1.5">
                      <label className="text-sm font-semibold text-secondary">New Password</label>
                      <input type="password" value={newPassword} onChange={(e) => setNewPassword(e.target.value)} placeholder="Enter new password" className="w-full rounded-xl border border-black/10 bg-gray-50/50 px-4 py-3 text-sm focus:border-primary focus:bg-white focus:outline-none focus:ring-4 focus:ring-primary/10" />
                    </div>
                  </div>
                  <div className="flex justify-start pt-6 border-t border-black/5">
                    <button onClick={() => handleSave("password")} disabled={!oldPassword || !newPassword || isSubmitting} className="btn-primary flex items-center gap-2"><Lock className="h-4 w-4" /> Update Password</button>
                  </div>
                </div>
              )}
