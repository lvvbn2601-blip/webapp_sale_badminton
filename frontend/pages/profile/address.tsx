...
export default function ProfileAddressPage() {
  ...
  const [activeTab, setActiveTab] = useState<"profile" | "address" | "password" | "purchases" | "notifications" | "vouchers">("address");
                ...
                <div className="p-6 sm:p-8 space-y-8 animate-in fade-in duration-300">
                  ...
                  <div className="space-y-4">
                    {addressList.length === 0 ? (
                      <p className="text-sm text-secondary/60">No addresses saved yet.</p>
                    ) : (
                      addressList.map((addr, idx) => (
                        <div key={idx} className={`p-4 rounded-xl border relative transition-colors ${primaryAddress === addr ? "border-primary bg-primary/5" : "border-black/5 bg-gray-50/50"}`}>
                          <div className="flex justify-between items-start gap-4">
                            <p className="text-sm text-secondary whitespace-pre-wrap flex-1">{addr}</p>
                            <div className="flex flex-col items-end gap-2">
                              {primaryAddress !== addr ? (
                                <button onClick={() => setPrimaryAddress(addr)} className="text-xs font-semibold text-primary hover:underline">Set Default</button>
                              ) : (
                                <span className="text-xs font-semibold text-white bg-primary px-2 py-1 rounded-md">Default</span>
                              )}
                              <button onClick={() => removeAddress(idx)} className="text-red-500 hover:bg-red-50 p-1.5 rounded-lg transition-colors" title="Remove Address">
                    ...
                    {isAddingAddress ? (
                      <div className="p-4 rounded-xl border border-black/10 bg-white space-y-3 animate-in slide-in-from-top-2">
                        <textarea
                          className="w-full rounded-xl border border-black/10 bg-gray-50/50 px-4 py-3 text-sm focus:border-primary focus:bg-white focus:outline-none focus:ring-4 focus:ring-primary/10 resize-none"
                          rows={3}
                        ...
                        <div className="flex justify-end gap-2">
                          <button onClick={() => setIsAddingAddress(false)} className="px-4 py-2 text-sm font-semibold text-secondary hover:bg-gray-100 rounded-lg">Cancel</button>
                          <button onClick={addAddress} className="btn-primary py-2 px-4 text-sm">Add</button>
                        </div>
                      </div>
                    ) : (
                      <button onClick={() => setIsAddingAddress(true)} className="flex items-center gap-2 text-sm font-semibold text-primary hover:underline group">
                        <span className="p-1 rounded-full bg-primary/10 group-hover:bg-primary/20 transition-colors"><Plus className="w-4 h-4" /></span>
                        Add New Address
                      </button>
                    )}
                  </div>
                  ...
                  <div className="flex justify-start pt-6 border-t border-black/5">
                    <button onClick={() => handleSave("address")} disabled={isSubmitting} className="btn-primary flex items-center gap-2"><Save className="h-4 w-4" /> Save Address Book</button>
                  </div>
                </div>
              )}
