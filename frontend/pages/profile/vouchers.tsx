...
import { fetchProfile, updateProfile, uploadImage, fetchUserOrders, fetchNotifications, markNotificationRead, markAllNotificationsRead, createReview, cancelUserOrder, confirmReceipt, requestReturn, fetchUserReviews, updateReview, fetchPublicCoupons } from "../../lib/api";
...
export default function ProfileVouchersPage() {
  ...
  const [activeTab, setActiveTab] = useState<"profile" | "address" | "password" | "purchases" | "notifications" | "vouchers">("vouchers");
  ...
  useEffect(() => {
    ...
    ]).then(([profileRes, ordersRes, reviewsRes, couponsRes]) => {
        ...
        <div className="container-default">
          ...
          <div className="grid gap-8 lg:grid-cols-[250px_1fr]">
            ...
            <div className="rounded-2xl bg-white shadow-sm ring-1 ring-black/5 overflow-hidden">
              ...
              {message.text && activeTab !== "purchases" && activeTab !== "notifications" && (
              ...
              {activeTab === "vouchers" && (
                <div className="p-6 sm:p-8 space-y-6 animate-in fade-in zoom-in-95 duration-300">
                  <div className="flex items-center justify-between border-b border-black/5 pb-6">
                    <div>
                      <h2 className="text-xl font-bold text-secondary">My Vouchers</h2>
                      <p className="text-sm text-secondary/60">Available coupons and promo codes for your next purchase.</p>
                    </div>
                  </div>
                  ...
                  {loadingCoupons ? (
                    ...
                    <div className="flex flex-col items-center justify-center py-20 text-center">
                      ...
                      <h2 className="text-xl font-bold text-secondary">No Vouchers Available</h2>
                      <p className="mt-2 text-sm text-secondary/60 max-w-sm">We'll let you know when you earn exclusive coupons or rewards.</p>
                    </div>
                  ) : (
                    <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-2">
                      {coupons.map((coupon, idx) => {
                        const progress = coupon.usageLimit
                          ...
                          : null;
                        const isUsedByMe = coupon.usedByCurrentUser;

                        return (
                          <div key={coupon._id || idx} className={`group relative flex flex-col overflow-hidden rounded-2xl bg-white shadow-md transition-all hover:-translate-y-1 hover:shadow-xl ${isUsedByMe ? "opacity-60 grayscale hover:-translate-y-0 hover:shadow-md" : ""}`}>

                            {/* Decorative Left/Right Ticket Cutouts */}
                            <div className="absolute -left-3 top-[72px] h-6 w-6 rounded-full bg-gray-50/50 border-r border-black/5 z-10 shadow-inner" />
                            <div className="absolute -right-3 top-[72px] h-6 w-6 rounded-full bg-gray-50/50 border-l border-black/5 z-10 shadow-inner" />

                            <div className={`relative flex items-center gap-5 border-b border-dashed border-black/10 p-6 
                              ${isUsedByMe ? "bg-gray-100" : "bg-gradient-to-r from-orange-50 to-primary/5"}`}>
                              {/* Background Pattern */}
                              {!isUsedByMe && <div className="absolute inset-0 opacity-[0.03]" style={{ backgroundImage: "radial-gradient(#000 1px, transparent 1px)", backgroundSize: "10px 10px" }} />}

                              <div className={`relative z-10 flex h-16 w-16 shrink-0 flex-col items-center justify-center rounded-2xl shadow-lg 
                                ${isUsedByMe ? "bg-gray-300 text-gray-500 shadow-none" : "bg-gradient-to-br from-primary to-orange-500 text-white shadow-primary/30"}`}>
                                <span className="text-xs font-extrabold uppercase leading-none opacity-80">
                              ...
                              <div className="relative z-10 flex-1">
                                <h3 className="line-clamp-1 text-lg font-black tracking-tight text-secondary">{coupon.code}</h3>
                                <p className="line-clamp-2 text-xs font-medium text-secondary/70 leading-tight mt-1">{coupon.program}</p>
                              </div>
                            </div>

                            <div className="flex flex-col gap-4 p-6 bg-white relative">
                              <div className="flex flex-wrap gap-2 text-xs">
                                <span className="inline-flex items-center gap-1 rounded-lg bg-gray-100/80 px-2.5 py-1.5 font-semibold text-secondary/70"><CheckCircle2 className="h-3.5 w-3.5 text-green-500" /> {coupon.minOrderValue ? `Min $${coupon.minOrderValue}` : "No Min. Spend"}</span>
                                {coupon.maxDiscount > 0 && <span className="inline-flex items-center gap-1 rounded-lg bg-gray-100/80 px-2.5 py-1.5 font-semibold text-secondary/70"><CheckCircle2 className="h-3.5 w-3.5 text-blue-500" /> Max ${(coupon.maxDiscount).toFixed(2)}</span>}
                                <span className="inline-flex items-center gap-1 rounded-lg bg-red-50 px-2.5 py-1.5 font-semibold text-red-600"><Clock className="h-3.5 w-3.5" /> {new Date(coupon.expiresAt).toLocaleDateString()}</span>
                              </div>

                              {progress !== null && (
                                <div className="space-y-1.5 relative mt-1">
                                  <div className="flex justify-between text-xs font-bold text-secondary/80">
                                    <span>Usage Progress</span>
                                    <span className="text-primary">{progress}%</span>
                                  </div>
                                  <div className="h-2 w-full overflow-hidden rounded-full bg-gray-100 shadow-inner">
                                    <div className="h-full rounded-full bg-gradient-to-r from-orange-400 to-primary transition-all duration-1000 ease-out" style={{ width: `${progress}%` }} />
                                  </div>
                                </div>
                              )}
                              ...
                              <div className="mt-2 text-center pointer-events-auto">
                                <button
                                  className={`w-full rounded-xl px-4 py-3 text-sm font-bold transition-all
                                    ${isUsedByMe ? "bg-gray-100 text-gray-400 cursor-not-allowed" : "bg-primary/10 text-primary hover:bg-primary hover:text-white hover:shadow-lg hover:shadow-primary/20"}`}
                                  disabled={isUsedByMe} onClick={() => {
                                    if (!isUsedByMe) {
                                  ...
                                  {isUsedByMe ? "Already Used" : "Copy Voucher Code"}
                                </button>
                              </div>
                            </div>
                          </div>
