...
export default function AdminBrandsPage() {
  ...
  const [section, setSection] = useState<AdminSection>("brands");
        ...
        <div className="space-y-6">
          <Panel title="Brand Management">
            <div className="grid gap-3 sm:grid-cols-[1fr_auto]">
              ...
              />
              <button className="btn-primary whitespace-nowrap" onClick={openCreateBrand}>
                + Add Brand
              </button>
            </div>
          </Panel>

          <Panel title={`Brands (${filteredBrands.length})`}>
            <div className="overflow-x-auto">
              <table className="w-full text-left text-sm">
                ...
                <tbody>
                  {pagedBrands.map((b) => (
                    <tr key={b._id || b.id} className="border-b border-black/5 transition-colors hover:bg-black/[0.02]">
                      <td className="py-3 pr-4">
                        {b.image ? (
                          ...
                          <div className="grid h-10 w-10 place-items-center rounded-xl bg-gradient-to-br from-primary/20 to-primary/5 text-xs font-bold text-primary">
                            {b.name.charAt(0).toUpperCase()}
                          </div>
                        )}
                      </td>
                      <td className="py-3 font-semibold text-secondary">{b.name}</td>
                      <td className="py-3 text-secondary/60">{b.slug}</td>
                      <td className="py-3 text-secondary/60">
                        <span className="line-clamp-1 max-w-[200px]">{b.description || "—"}</span>
                      </td>
                      <td className="py-3 text-right">
                        <div className="flex items-center justify-end gap-2">
                          ...
                          <button
                            ...
                            onClick={() => removeBrand(b._id || b.id)}
                          >
          ...
          {brandFormOpen && (
            <ModalOverlay onClose={() => setBrandFormOpen(false)}>
              <div className="w-full max-w-lg rounded-3xl bg-white p-6 shadow-2xl">
                <div className="flex items-center justify-between">
                  <h3 className="font-heading text-xl font-semibold text-secondary">
                    {brandForm.id ? "Edit Brand" : "Create Brand"}
                  </h3>
                  <button className="pill" onClick={() => setBrandFormOpen(false)}>
                    Close
                  </button>
                </div>

                <form className="mt-5 space-y-4" onSubmit={saveBrand}>
                  <div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setBrandForm((p) => ({ ...p, image: e.target.value }))}
                    />
                    {brandForm.image && (
                  ...
                  <div className="flex items-center justify-end gap-3 pt-2">
                    ...
                    <button className="btn-primary" disabled={brandSaving}>
                      {brandSaving ? "Saving..." : brandForm.id ? "Update Brand" : "Create Brand"}
                    </button>
                  </div>
                </form>
              </div>
